...
void main()
{
    ...
    // Grass blades face camera, so use a simple up-facing normal
    // rotated slightly based on the blade orientation
    vec3 bladeNormal = vec3(0.0, 1.0, 0.0);
    vViewNormal = normalize(normalMatrix * bladeNormal);
}
